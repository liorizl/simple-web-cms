...
module.exports={
    ...
    upArticle:async ctx=>{
        ...
        await serverUtil.getForm(ctx.req).then(async value=>{
            let sql,myStatus,build,insertId;
            if(act==='add'){
                let artName=value.articleName[0],articleName,articleIntro;
                ...
                articleIntro=value.intro[0]||util.regexpContent(value.content[0]).substr(0,100);
                sql='insert into article(fid,title,mainTitle,viceTitle,isIndex,isUse,headLine,suggest,outUrl,keywords,content,intro,picUrl,path,articleName,author,source,sourceUrl,useSourceUrl,'+
                        ...
                        parseInt(value.headLine[0])+','+parseInt(value.headLine[0])+',"'+value.outUrl[0]+'",keywords="'+value.keywords[0]+'",'+
                ...
                sql='update article set title="'+value.title[0]+'",mainTitle="'+value.mainTitle[0]+'",viceTitle="'+value.viceTitle[0]+'",isIndex="'+serverUtil.getCheckbox(value.isIndex[0])+'",'+
                        'isUse="'+serverUtil.getCheckbox(value.isUse[0])+'",content="'+util.regexpContent(value.content[0])+'",intro="'+value.intro[0]+'",picUrl="'+value.picUrl[0]+'",'+
