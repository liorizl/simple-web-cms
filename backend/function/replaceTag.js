                ...
                const rShortTag=(tempStr,reSqlObj,parent)=>{
                    ...
                    if(resReg){
                        ...
                        switch (field){
                            case 'title':
                                tempStr=argArr[1]?
                                        tempStr.replace(resReg[0],reSqlObj[field].substring(0,argArr[1])):
                                        tempStr=tempStr.replace(resReg[0],reSqlObj[field]);
                                break;
                            case 'intro':
                                tempStr=argArr[2]?
                                        tempStr.replace(resReg[0],reSqlObj[field].substring(0,argArr[2])):
                                        tempStr=tempStr.replace(resReg[0],reSqlObj[field]);
                                break;
                            case 'colurl':  //栏目数据库的路径前面没有'/',文章的路径有'/'
                                if(reSqlObj['outurl']){
                                    tempStr=tempStr.replace(resReg[0],reSqlObj['outurl']);
                                }
                                else if(webset&&webset.pagemodel===1){
                                    tempStr=tempStr.replace(resReg[0],util.repalceStr2(webset.pagepath,'col')+reSqlObj['path1']+'/'+reSqlObj['path2']);
                                }
                                else if(webset&&webset.pagemodel===2){
                                    tempStr=tempStr.replace(resReg[0],util.repalceStr(webset.weburl)+'/showColumn?id='+reSqlObj['id']);
                                }
                                else{
                                    tempStr=tempStr.replace(resReg[0],'/showColumn?id='+reSqlObj['id']);
                                }
                                break;
                            case 'arturl':  //栏目数据库的路径前面没有'/',文章的路径有'/'
                                if(reSqlObj['outurl']){
                                    ...
                                    if(webset&&webset.pagemodel===1){
                                        tempStr=tempStr.replace(resReg[0],util.repalceStr(webset.weburl)+util.repalceStr2(webset.pagepath,'art')+reSqlObj['path']+'/'+reSqlObj['articlename']+'.html');
                                    }
                                    else if(webset&&webset.pagemodel===2){
                                        tempStr=tempStr.replace(resReg[0],util.repalceStr(webset.weburl)+'/showArticle?id='+reSqlObj['id']);
                                    }
                                    else{
                                        tempStr=tempStr.replace(resReg[0],'/showArticle?id='+reSqlObj['id']);
                                    }
                                }
                                break;
                            case 'uptime':
                                if(argArr[4]){
                                    tempStr=tempStr.replace(resReg[0],util.formatToDate(reSqlObj[field],argArr[4]));
                                }else{
                                    tempStr=tempStr.replace(resReg[0],reSqlObj[field])
                                }
                                break;
                            case 'pagelist':
                                if(args[3]==='build'&&argArr[0]>0&&argArr[3]>0){
                                    tempStr=tempStr.replace(resReg[0], util.makePaging(argArr[0],args[1],args[0],argArr[4],args[2],args[3]));
                                }
                                else if(argArr[0]>0&&argArr[3]>0){
                                    tempStr=tempStr.replace(resReg[0], util.makePaging(argArr[0],args[1],args[0],argArr[4],args[2]));
                                }
                                else{
                                    tempStr=tempStr.replace(resReg[0], '该分页没有设置显示条数或者分页样式！');
                                }
                                break;
                            case 'webkeyword':
                                if(webset){
                            ...
                            case 'webdescription':
                                if(webset){
                                    tempStr=tempStr.replace(resReg[0],webset.description);
                                }else{
                                    tempStr=tempStr.replace(resReg[0],resReg[1]);
                                }
                                break;
                            case 'artdescription':
                                if(reSqlObj.description){
                                    tempStr=tempStr.replace(resReg[0],reSqlObj.description);
                                }else{
                                    tempStr=tempStr.replace(resReg[0],reSqlObj.intro.replace(/\<.*?\>/g,''));
                                }
                                break;
                            case 'webtitle':
                                if(webset){
                                    tempStr=tempStr.replace(resReg[0],webset.webname);
                                }else{
                                    tempStr=tempStr.replace(resReg[0],resReg[1]);
                                }
                                break;
                            case 'keywords':
                                if(!reSqlObj.keywords&&webset.keyword){
                                    ...
                                    tempStr=tempStr.replace(resReg[0],reSqlObj.keywords);
                                }
                                else{
                                    tempStr=tempStr.replace(resReg[0],field);
                                }
                                break;
                            case 'author':
                                if(reSqlObj[field]){
                                    tempStr=tempStr.replace(resReg[0],'作者:'+reSqlObj[field]);
                                }
                                else{
                                    tempStr=tempStr.replace(resReg[0],'');
                                }
                                break;
                            case 'source':
                                if(reSqlObj[field]){
                                    if(reSqlObj['usesourceurl']==='true'){
                                        tempStr=tempStr.replace(resReg[0],'来源：<a href="'+reSqlObj['sourceurl']+'" target="_blank">'+reSqlObj['source']+'</a>');
                                    }else{
                                        tempStr=tempStr.replace(resReg[0],'来源：'+reSqlObj['source']);
                                    }
                                }else{
                                    tempStr=tempStr.replace(resReg[0],'');
                                }
                                ...
                                if(parentField){
                                    if(parent&&(parent[parentField[1]]||parentField[1]==='url')){
                                        if(parentField[1]==='url'){
                                            if(webset&&webset.pagemodel===1){
                                                tempStr=tempStr.replace(resReg[0],util.repalceStr2(webset.pagepath,'col')+parent['path1']+'/'+parent['path2']);
                                            }
                                            else if(webset&&webset.pagemodel===2){
                                                tempStr=tempStr.replace(resReg[0],util.repalceStr(webset.weburl)+'/showColumn?id='+parent['id']);
                                            }
                                            else{
                                                tempStr=tempStr.replace(resReg[0],'/showColumn?id='+parent['id']);
                                            }
                                        }
                                        tempStr=tempStr.replace(resReg[0],parent[parentField[1]]);
                                    }else{
                                        tempStr=tempStr.replace(resReg[0],parentField[0]);
                                    }
                                }else{
                        ...
                        //     tempStr=tempStr.replace(resReg[0],reSqlObj[resReg[1]].substring(0,argArr[2]));
                        // }
