    ...
    <div class="container">
        <div class="left">
            <ul class="singleCol">
                ...
                <li @click="buildHitsJs('all')">生成hitsJS</li>
                <li @click="buildBannerJs('all')">生成bannerJS</li>
            </ul>
        </div>
        <div class="line"></div>
        <div class="right">
    ...
    methods: {
        ...
        buildHitsJs() {
            this.axios({
                ...
                if (res.status === 200) {
                    this.buildJsMes = '<span style="color:red">生成hitsJS文件完毕</span><br>名称：' + res.data.name + '   目录： ' + res.data.path + '<br>' +
        ...
        buildBannerJs() {
            this.axios({
                ...
                if (res.status === 200) {
                    this.buildJsMes = '<span style="color:red">生成bannerJS文件完毕</span><br>名称：' + res.data.name + '   目录： ' + res.data.path + '<br>' +
                        '用途:banner轮播JS（静态页面下避免修改banner重新生成页面）<br>' +
                        '<span style="color:red">需要在页面头部导入swiper.min.js和swiper.min.css</span> <br>' +
                        '引用地址:http://' + this.config.hostName + ':' + this.config.port + '/static/js/' + res.data.name + '(注意没有/statics)' +
                        '<br>也可以将到复制到你的网站静态文件夹下面再引入<br>' +
                        '如果网站静态文件目录里面已经有此目录+文件将会优先访问网站目录下的'
                }
